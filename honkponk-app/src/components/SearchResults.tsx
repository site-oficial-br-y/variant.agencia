...
const BRAZIL_CITIES = [
  ...
  { lat: -8.05,  lng: -34.88, name: 'Recife' },
  { lat: -15.78, lng: -47.93, name: 'Brasília' },
  { lat: -1.46,  lng: -48.50, name: 'Belém' },
  { lat: -3.10,  lng: -60.02, name: 'Manaus' },
]

async function fetchPlacesBrazil(
  query: string,
  onProgress: (city: string, done: number) => void
): Promise<any[]> {
  const results: any[][] = new Array(BRAZIL_CITIES.length).fill([])
  let done = 0
  await Promise.all(
    BRAZIL_CITIES.map(async (c, i) => {
      const batch = await fetchPlaces(query, c.lat, c.lng, 30000)
      results[i] = batch
      done++
...
function isHotLead(place: PlaceResult): boolean {
  return place.rating >= 4.5 && place.reviews >= 30
}
...
export function SearchResults({ params, userId, plan = 'free', onLimitReached }: { params: SearchParams; userId: string | null; plan?: Plan; onLimitReached: () => void }) {
  ...
  if (loading) {
    const totalCities = BRAZIL_CITIES.length
    const pct = params.allBrazil ? Math.round((progressDone / totalCities) * 100) : 60
    return (
      <div style={{ padding: '40px 0' }}>
        <div style={{ marginBottom: 28, padding: '20px 24px', background: 'rgba(232,121,160,.06)', border: '1px solid rgba(232,121,160,.15)', borderRadius: 16 }}>
          ...
          {/* Progress bar */}
          <div style={{ height: 6, background: 'rgba(255,255,255,.08)', borderRadius: 4, overflow: 'hidden', marginBottom: 10 }}>
            <div style={{ height: '100%', width: `${pct}%`, background: 'linear-gradient(90deg,#e879a0,#c2185b)', borderRadius: 4, transition: 'width .4s ease' }} />
          </div>
          <div style={{ fontSize: '.75rem', color: 'rgba(255,255,255,.4)' }}>
            {params.allBrazil
              ? (progressCity ? `✓ ${progressCity}` : 'Iniciando varredura…')
              : `Pesquisando em ${params.city}…`}
          </div>
        </div>
        {[1,2,3].map(i => (
          <div key={i} style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 16, padding: '18px 22px', marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, opacity: 1 - i * 0.22 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: 11, width: '75%', background: 'rgba(255,255,255,.05)', borderRadius: 6, marginBottom: 10 }} />
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ background: 'linear-gradient(135deg,#e879a0,#c2185b)', borderRadius: 8, padding: '6px 14px', fontSize: '.82rem', fontWeight: 800, letterSpacing: .5, minWidth: 80, textAlign: 'center' }}>
            {displayCount} leads
          </div>
          ...
          {results.map((place, i) => {
            ...
            return (
              <div key={i} className="lead-card" style={{
                background: hot ? 'rgba(251,191,36,.04)' : 'rgba(255,255,255,.03)',
                border: `1px solid ${hot ? 'rgba(251,191,36,.25)' : 'rgba(248,182,200,.1)'}`,
                borderRadius: 16, padding: '16px 20px',
                display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap',
                transition: 'border-color .2s, background .2s',
                animation: `fadeUp .4s ease ${Math.min(i * 0.06, 0.4)}s both`,
                position: 'relative',
              }}>
                {hot && (
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flex: 1, minWidth: 200 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 10, background: hot ? 'rgba(251,191,36,.15)' : 'rgba(232,121,160,.1)', border: `1px solid ${hot ? 'rgba(251,191,36,.3)' : 'rgba(232,121,160,.2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.72rem', fontWeight: 800, color: hot ? '#fbbf24' : '#e879a0', flexShrink: 0 }}>
                    {i + 1}
                  </div>
                  <div style={{ flex: 1 }}>
